...
class DatabaseManager:
    ...
    def __validate_query(self, query, query_type):
        ...
        if command != valid_commands.get(query_type, "").upper():
            raise ValueError(
                f"Query {query_type} is not valid or secure.")

        if re.search(r"(--|\bDROP\b|\bTRUNCATE\b)",
                     query,
                     re.IGNORECASE):
            raise ValueError(
                "The query contains dangerous keywords.")

    def __execute_query(self, query, query_type):
